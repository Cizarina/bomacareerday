...
const TEAM_HEADERS = ["id", "name", "phone", "email", "role", "zone", "cluster", "status", "notes", "updatedAt", "accessLevel", "pin", "mode", "sessionLink", "classStream", "shifts"];
...
const CHAT_HEADERS = ["id", "timestamp", "who", "message"];
...
const PRIVATE_CHAT_HEADERS = ["id", "timestamp", "fromId", "fromName", "toId", "toName", "message", "readByRecipient"];
...
const GROUP_CHAT_HEADERS = ["id", "timestamp", "groupId", "who", "whoId", "message"];
...
// is on file. All four appended at the END on purpose, same reasoning as
// STUDENTS_HEADERS above — keeps position alignment on a live sheet.
...
function visibleTeam_(me, allTeam) {
  ...
  if (me.accessLevel === "zone") {
    const zoneLetter = zoneLetterOf_(me.zone);
    return allTeam.filter(function(t) {
...
function doPost(e) {
  try {
    ...
    const ADMIN_ONLY = ["update_access", "run_allocation", "resend_pin", "approve_mentor_application", "reject_mentor_application"];
...
function addTeamMember_(body, requester) {
  ...
  const safeDefault = body.role === "Class Teacher" ? "class" : "cluster";
...
function emailPinIfPossible_(email, name, pin) {
  ...
  try {
    MailApp.sendEmail({
      ...
      body:
        "Hi " + (name || "") + ",\n\n" +
        "Here's how to sign in to Boma Career Day — WG2 Hub:\n\n" +
        "Name: " + (name || "") + "\n" +
        "PIN: " + pin + "\n\n" +
        "You can change this PIN any time after signing in — tap your name at the top of the app.\n\n" +
        "WG2 Boma Career Day 2026",
      name: "WG2 Boma Career Day 2026",
    });
    return true;
  } catch (err) {
    return false;
...
function publicRegisterMentor_(body) {
  ...
  if (exbomarian !== "Yes" && exbomarian !== "No") return { ok: false, error: "Please answer the Exbomarian question." };
...
function approveMentorApplication_(body, me) {
  ...
  const cluster = String(body.cluster || app.primaryCluster || "").trim().toUpperCase();
  const clusters = readSheet_(CLUSTERS_SHEET, CLUSTERS_HEADERS);
  ...
  const notes = "From mentor application " + app.id + ". " + noteBase + "Shifts: " + (app.shifts || "—") + ".";
  ...
  logActivity_(me.name, "approve_mentor_application", app.id, "-> " + newTeamId);
  ...
  const emailSent = emailMentorApproval_(app, pin, clusterRow);
  return { ok: true, teamMemberId: newTeamId, pin: pin, emailSent: emailSent };
}
...
const CLUSTER_KEYWORDS_ = {
  ...
  E3: ["fashion design","graphic design","artist","photograph","musician","dance","drama","film","videograph","creative director","cosmetolog","salon owner","stylist","dj ","styling"],
...
function emailMentorApproval_(app, pin, clusterRow) {
  ...
  try {
    MailApp.sendEmail({
      ...
      body:
        ...
        "Here's how to sign in to the Boma Career Day — WG2 Hub app to see your schedule and details:\n\n" +
        "Name: " + (app.name || "") + "\n" +
        "PIN: " + pin + "\n\n" +
        "You can change this PIN any time after signing in — tap your name at the top of the app.\n\n" +
...
function sendSegmentEmail_(body, me) {
  ...
  if (segmentType === "class") {
    ...
    if (images.length) {
      ...
      MailApp.sendEmail({ to: teacherEmail, subject: subject, body: message, name: "WG2 Boma Career Day 2026" });
    }
  ...
  MailApp.sendEmail({ to: sender, bcc: uniqueEmails.join(","), subject: subject, body: message, name: "WG2 Boma Career Day 2026" });
...
function emailOwnQr_(body) {
  ...
  try {
    ...
    GmailApp.sendEmail(to, "Your WG2 Boma Career Day 2026 QR code",
      ...
      { htmlBody: htmlBody, inlineImages: { qr: blob }, name: "WG2 Boma Career Day 2026" });
...
function postChat_(body) {
  ...
  sheet.appendRow([newId, new Date().toISOString(), body.who || "", msg]);
  return { ok: true, id: newId };
}
...
function sendPrivateMessage_(body, me) {
  ...
  sheet.appendRow([newId, new Date().toISOString(), me.id, me.name, toId, recipient.name, msg, "No"]);
  return { ok: true, id: newId };
}
...
function postGroupMessage_(body, me) {
  ...
  let newId = "GM" + String(n).padStart(5, "0");
  while (ids.indexOf(newId) !== -1) { n++; newId = "GM" + String(n).padStart(5, "0"); }
  sheet.appendRow([newId, new Date().toISOString(), groupId, me.name, me.id, msg]);
  return { ok: true, id: newId };
}
